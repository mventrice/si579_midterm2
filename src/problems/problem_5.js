...
function startAnimation(blockEl) {
  // blockEl is a DOM element.
  const duration = 2000; // The animation runs for 2 seconds
  const startX = 0; // the block moves from x===0...
  const endX = 500; // to x===500

  function setX(x) {
    blockEl.style.setProperty('left', `${x}px`);
  } // A function to set the x position of the block
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={() => startAnimation(blockRef.current)}
      >
        Start
      </button>
    </>
